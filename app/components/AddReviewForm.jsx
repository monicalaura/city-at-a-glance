...
export default function AddReviewForm({ cityName }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/reviews/${encodeURIComponent(cityName)}`, {
        method: "POST",
        headers: {
    ...
    <div className="container mx-auto my-8 flex justify-center items-center">
       {review ? (
        <div className="max-w-full lg:max-w-full">
          <h2 className="text-brand-primary text-2xl font-bold mb-4 text-center">Your Review</h2>
          <div className="text-center controls">

            <Link
                href={`/edit-review/${review._id}`}
                title="Edit">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="max-w-lg w-full text-center p-8 bg-white shadow-md rounded-md">
          <h2 className="text-brand-primary text-2xl font-bold mb-4">Your Review</h2>
          <textarea
            id="review"
            name="review"
            rows="4"
